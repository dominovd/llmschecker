...
// Default: throws if it exceeds maxBytes.
// With { truncate: true }: reads up to maxBytes and returns the partial text
// (used for crawling large sitemaps/pages, where partial content is still useful).
export async function readCappedText(res, maxBytes = 5 * 1024 * 1024, { truncate = false } = {}) {
  const lenHeader = res.headers.get("content-length");
  if (!truncate && lenHeader && Number(lenHeader) > maxBytes) {
  ...
  while (true) {
    ...
    if (received >= maxBytes) {
      ...
      break; // truncate: keep what we have and stop
    }
  }
