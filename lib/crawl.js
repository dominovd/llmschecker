...
async function discoverCandidates(origin, providedSitemaps = []) {
...
export async function crawlSite(domain, options = {}, onProgress = () => {}) {
  ...
  const { urls, stats } = await discoverCandidates(origin, providedSitemaps);
  ...
  const pages = (
    await pool(ranked.map((url, i) => ({ url, id: `p${i + 1}` })), worker, LIMITS.concurrency)
  ).filter(Boolean);
